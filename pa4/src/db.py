...
# your classes here
association_table = db.Table('association', db.Column('course_id', db.Integer, db.ForeignKey('courses.id')), db.Column('user_id', db.Integer, db.ForeignKey('users.id')))
class Course(db.Model):
    ...
    students=db.relationship("User", secondary=association_table, back_populates="courses")
    instructors=db.relationship("User", secondary=association_table, back_populates="courses")
...
class User(db.Model):
  ...
  courses1=db.relationship("Course", secondary=association_table, back_populates="students")
  def __init__(self, **kwargs):
    """
  ...
  def serialize(self):
    ...
    """
    return {
      "id": self.id, "name":self.name, "netid":self.netid, "courses":[c.simple_serialize() for c in self.courses]+[d.simple_serialize() for d in self.courses1],}
